  ...
  if ($var9>0){
	...
	while (odbc_fetch_row($rs2)) {
		...
		$OW++;
		 ...
		 //add perf data for well to OW_array (1 dimensional)
		 /*
		$OW_array[$var9.'-'.$OW.'TOP'] = ($var13);
		$OW_array[$var9.'-'.$OW.'BASE'] = ($var14);
		$OW_array[$var9.'-'.$OW.'DATE'] = ($var12);
		$OW_array[$var9.'-'.$OW.'TYPE'] = ($var15);
		*/
		
		//add perf data for well as a line item
		$OW_perf_line_item_array[$OW] = ($var13."-".$var14."-".$var12."-".$var15);
		echo $var13."-".$var14."-".$var12."-".$var15;
		//$OW_array[] = ( $var9.'-'.$OW.'TOP' => $var13, $var9.'-'.$OW.'BASE' => $var14, $var9.'-'.$OW.'DATE' => $var12, $var9.'-'.$OW.'TYPE' => $var15);
		 
		//add the variables to a string that can be exported to csv
		$txtOW .= $var9.",". $var8.",".$var13.",".$var14.",".$var12.",".$var15.",OW
		";
	...
	while (odbc_fetch_row($rs3)) {
		...
		//add perf data for well to PETRA_array (2 dimensional)
		/*
		$PETRA_array[$var9.'-'.$PETRA.'TOP'] = ($var23);
		$PETRA_array[$var9.'-'.$PETRA.'BASE'] = ($var24);
		$PETRA_array[$var9.'-'.$PETRA.'DATE'] = ($var22);
		$PETRA_array[$var9.'-'.$PETRA.'TYPE'] = ($var27);
		*/

		//add perf data for well as a line item
		$PETRA_perf_line_item_array[$PETRA] = ($var23."-".$var24."-".$var22."-".$var27);
		echo $var23."-".$var24."-".$var22."-".$var27;
		//$PETRA_array[] = $var9.'-'.$PETRA.'TOP' => $var13, $var9.'-'.$PETRA.'BASE' => $var14, $var9.'-'.$PETRA.'DATE' => $var12, $var9.'-'.$PETRA.'TYPE' => $var15);
		
		//add the variables to a string that can be exported to csv
		$txtPETRA .= $var9.",". $var8.",".$var23.",".$var24.",".$var22.",".$var27.",PETRA
		";
...
echo "<br>perfect matches on order or values: ".$perfect_match;
echo "<br>conflicting on order or values: ".$conflicting;
