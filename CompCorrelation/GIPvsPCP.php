<script type="text/javascript">
    ...
    google.load("visualization", "1", {packages:["corechart"]});
		...
		data.addColumn('number', 'GIP OPSII BCF per 10 acre'); // Implicit domain label col.
		...
		data.addRows([
			...
			for ($row = 0; $row < count($wellname_array)-1; $row++) {
				if ( $SUMGIP2_gross[$row] > 0 && $PCP_array[$row] >0) {
				echo "[".$SUMGIP2_gross[$row].",";
				echo $PCP_array[$row].", ";
			...
			var options = {
				 title: 'GIP2 vs. Relative Well Performance',
				annotations: {
					textStyle: {
					  fontName: 'open sans',
					  fontSize: 14,
				...
				hAxis: {
				    title: "GIP OPSII BCF per 10 acre", 
					//logScale: 'true',
					//format:  "####",
				},
