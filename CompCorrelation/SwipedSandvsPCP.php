<script type="text/javascript">
    ...
    google.load("visualization", "1", {packages:["corechart"]});
		...
		data.addColumn('number', 'Gross Swiped Sand'); // Implicit domain label col.
		...
		data.addRows([
			...
			for ($row = 0; $row < count($wellname_array)-1; $row++) {
				...
				if ( $gross_swiped_sand_array[$row] > 0 && $PCP_array[$row] >0) {
				echo "[".$gross_swiped_sand_array[$row].", ";
				echo $PCP_array[$row].",";
			...
			var options = {
				title: 'Gross Swiped Sand vs. Relative Well Performance',
				annotations: {
					textStyle: {
					  fontName: 'open sans',
					  fontSize: 14,
			...
			var chart = new google.visualization.LineChart(document.getElementById('chart_GrossSwipedSandvsPCP'));
